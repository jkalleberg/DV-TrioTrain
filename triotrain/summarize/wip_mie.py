...
from typing import List, TextIO, Unio)om regex import compile
...
class MIE:
    ...
    def load_pedigree(self) -> None:
        """read in a pedigree CSV file"""
        if self.args.pedigree:
            self._pedigree_file = TestFile(self.args.pedigree, self.logger)
            self._pedigree_file.check_existing()

            if self._pedigree_file.file_exists:
                pedigree_data = list()

                with open(self._pedigree_file.file, newline="") as csvfile:
                    reader = DictReader(csvfile)
                    for row in reader:
                        pedigree_data.append(row)

                self._pedigree_groups = list(zip(*(iter(pedigree_data),) * 3))
            else:
                self.logger.error(
                    f"{self._stats._logger_msg}: missing the pedigree file | '{self._pedigree_file.file}'\nExiting..."
                )
                exit(1)

    def set_threshold(self) -> None:
        """Determines if concordance threshold devates from default"""
    ...
    def load_variables(self) -> None:
        ...
        if match:
            ...
            self.logger.error(
                f"{self._stats._logger_msg}: unable to identify a valid run number\nExiting..."
            )
            exit(1)
        ...
        self._total_num_tests = int(num_tests)
        ...
        assert (
            getcwd() == code_path
        ), f"{self._stats._logger_msg}: run the workflow in the {code_path} directory only!"

    def find_sampleIDs(self, vcf: TestFile) -> None:
        """Enables replacing 'Test##' with the sampleID from the orignal TruthVCF"""
        if vcf.file_exists:
            sample_names_cmd = ["bcftools", "query", "--list-samples", vcf.file]

            self._stats.execute(
                command_list=sample_names_cmd,
                type="bcftools query",
                run_interactive=True,
                keep_output=True,
            )

    def find_truthIDs(self) -> None:
        """_summary_"""
        self._truthIDs = dict()
        for t in range(0, self._total_num_tests):
            test_num = t + 1
            if self.args.debug:
                self.logger.debug(
                    f"{self._stats._logger_msg}: working on test #{test_num}..."
                )

            vcf = (
                Path(self.env.contents[f"Test{test_num}TruthVCF_Path"])
                / self.env.contents[f"Test{test_num}TruthVCF_File"]
            )
            vcf_file = TestFile(vcf, self.logger)
            vcf_file.check_existing()

            if vcf_file.file_exists:
                self.find_sampleIDs(vcf_file)
                num_of_outputs = len(self._stats._output_file_contents)
                if num_of_outputs == 1:
                    self._truthIDs[
                        f"Test{test_num}"
                    ] = self._stats._output_file_contents[0]
                else:
                    self.logger.error(
                        f"{self._stats._logger_msg}: unexpected number of samples in TruthVCF, expected 1 per file, but found | '{num_of_outputs}'\nExiting..."
                    )
                    exit(1)

    def find_trios(self) -> None:
        """Determine if pedigree file contains any 'Test##' genomes"""
        self._valid_trios = dict()
        self._use_these = list()
        for i, ped in enumerate(self._pedigree_groups):
            trio_num = i + 1
            _valid_trios = list()
            if self.args.debug:
                self.logger.debug(
                    f"{self._stats._logger_msg}: processing pedigree {trio_num}-of-{len(self._pedigree_groups)}:"
                )
            for j, sample in enumerate(ped):
                labeled_sample = dict()
                if self.args.debug:
                    self.logger.debug(
                        f"{self._stats._logger_msg}: \tsample #{j}: {sample}"
                    )
                matches = set(sample.values()) & set(self._truthIDs.values())
                found_ID = list(matches)[0]
                found_TestName = [
                    k for k, v in self._truthIDs.items() if v == str(found_ID)
                ]
                labeled_sample["TestName"] = found_TestName[0]
                self._use_these.append(found_TestName[0].lower())
                labeled_sample.update(sample)
                _valid_trios.append(labeled_sample)

            if len(_valid_trios) == 3:
                self._valid_trios[f"Trio{trio_num}"] = _valid_trios

    def find_test_vcfs(self, search_path: Path) -> None:
        """_summary_"""
        test_regex = compile(r"^test\d+(.*?)\.vcf\.gz$")

        test_files_exists, num_found, files_found = check_if_output_exists(
            match_pattern=test_regex,
            file_type="test VCF file",
            search_path=search_path,
            msg=self._stats._logger_msg,
            logger=self.logger,
            debug_mode=self.args.debug,
        )
        print("FILES FOUND:", files_found)
        print("USE THESE:", self._use_these)
        self._files_found = [f"{search_path / f}" for f in files_found if any(f.startswith(u) for u in self._use_these)]

    def find_testIDs(self) -> None:
        """_summary_"""
        self._testIDs = dict()
        for t in self._files_found:
            vcf_file = TestFile(t, self.logger)
            vcf_file.check_existing()

            if vcf_file.file_exists:
                test_name = remove_suffixes(vcf_file.path, remove_all=True)

                self.find_sampleIDs(vcf_file)
                num_of_outputs = len(self._stats._output_file_contents)
                if num_of_outputs == 1:
                    self._testIDs[test_name.name.capitalize()] = self._stats._output_file_contents[0]
                else:
                    self.logger.error(
                        f"{self._stats._logger_msg}: unexpected number of samples in TruthVCF, expected 1 per file, but found | '{num_of_outputs}'\nExiting..."
                    )
    ...
    def run(self) -> None:
        ...
        self.find_trios()
        if self._run_num == 0:
            self.find_test_vcfs(search_path=self._output_path)
            self.find_testIDs()
            print("TEST IDS:", self._testIDs)
            print("TRUTH IDS:", self._truthIDs)
            breakpoint()
        else:
            # iterate through both rounds of training
            for p in [self._test_mother, self._test_father]:
                self.find_test_vcfs(search_path=p)
                self.find_testIDs()
        breakpoint()
