...
example:
    python3 create_environment.py                   \\
        --first-genome Father                       \\
        --name <name_of_analysis_directory>         \\
        --metadata </path/to/metadata_file.csv>     \\
...
def collect_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--metadata",
        dest="metadata",
        help="[REQUIRED]\ninput file (.csv)\ndescribes each trio to analyze and the training order",
        type=str,
        metavar="</path/file>",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dry-run",
        dest="dry_run",
        help="display environment variables to the screen",
        action="store_true",
    )
    parser.add_argument(
        "-E",
    ...
    parser.add_argument(
        ...
        "--iteration",
        dest="itr_num",
        help="create an environment file for a single iteration",
        type=int,
        metavar="<int>",
    )
    parser.add_argument(
    ...
    #         "1",
    #         "--custom-checkpoint",
    #         "/storage/hpc/group/UMAG_test/WORKING/jakth2/deep-variant/pretrained_models/DV1.4_WGS.AF_cattle1/model.ckpt-203095",
    #         # "--debug",
    #         "--dry-run",
    #     ]
    # )
...
def check_args(args: argparse.Namespace, logger: Logger):
    ...
    _phase = "check_args"
    if args.debug:
        str_args = f"[{_phase}] - command line arguments: "
...
def __init__():
    ...
    new_data = Environment(
        ...
        logger,
        itr_num=iter,
        first_genome=args.first_genome,
        expected_num_tests=args.expected_num_tests,
