...
class CompareHappy:
    ...
    def __post_init__(self) -> None:
        ...
        self._convert_happy_dependencies = h.create_deps(self.itr.total_num_tests) 
    ...
    def find_restart_jobs(self) -> None:
        ...
        """
        self._ignoring_call_variants = h.check_if_all_same(
            self.call_variants_jobs, None
        )
        if not self._ignoring_call_variants:
            self._num_to_ignore = len(h.find_NaN(self.call_variants_jobs))
            select_ckpt_run = h.find_not_NaN(self.call_variants_jobs)
            if select_ckpt_run:
            ...
            if num_job_ids == self.itr.total_num_tests:
                self.jobs_to_run = h.find_not_NaN(self.compare_happy_job_nums)
                self._num_to_run = len(self.jobs_to_run)
                self._num_to_ignore = len(h.find_NaN(self.compare_happy_job_nums))
    ...
    def make_job(self, index: int = 0) -> Union[SBATCH, None]:
        ...
        # and when testing a new model, limit the regions evaluated with hap.py        
            ...
            regions = h.TestFile(Path(regions_file_path) / regions_file_name, logger=self.itr.logger)
    ...
    def check_submissions(self) -> None:
        ...
        if (
            ...
            sys.exit(1)
        compare_results = h.check_if_all_same(self._convert_happy_dependencies, None)
    ...
    def run(self) -> Union[List[Union[str, None]], None]:
        ...
        # determine if we should avoid certain tests because they are currently running
            ...
            if self._num_to_run == 0:
                ...
                if (
                    self._convert_happy_dependencies
                    and h.check_if_all_same(self._convert_happy_dependencies, None)
                    is False
                ):
                    self.itr.logger.info(
